...
class HospitallistPage extends StatelessWidget {
  ...
  Stream<QuerySnapshot> getDoctorsByCategory() {
    if (category == 'hospital') {
      // Fetch all hospitals, order by name (not rating)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        centerTitle: true,
        title: Text(
          category == 'hospital' ? 'Hospitals' : category,
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(
                child: Text(category == 'hospital'
                    ? 'No hospitals found'
                    : 'No doctors found for $category'));
          }
          final docs = snapshot.data!.docs;
          if (category == 'hospital') {
            // Sort hospitals: rated first (by rating desc), then unrated
            List<QueryDocumentSnapshot> sortedDocs = List.from(docs);
            sortedDocs.sort((a, b) {
              final aRating = (a.data() as Map<String, dynamic>)['rating'];
              final bRating = (b.data() as Map<String, dynamic>)['rating'];
